...
mock.onPost("/api/admin/v1/experience").reply((config) => {
...
mock.onGet("/api/admin/v1/experiences").reply((config) => {
    return [
        200,
        {
            code: RESPONSE.CODE.SUCCESS,
...
mock.onGet(new RegExp("/api/admin/v1/experience/(.+)")).reply((config) => {
    const experienceId = new RegExp("/api/admin/v1/experience/(.+)").exec(
        config.url!
    )![1]
...
mock.onDelete(new RegExp("/api/admin/v1/experience/(.+)")).reply((config) => {
    const experienceId = new RegExp("/api/admin/v1/experience/(.+)").exec(
        config.url!
    )![1]
...
mock.onPut(new RegExp("/api/admin/v1/experience/(.+)")).reply((config) => {
    const data = JSON.parse(config.data)
    const experienceId = new RegExp("/api/admin/v1/experience/(.+)").exec(
        config.url!
    )![1]
